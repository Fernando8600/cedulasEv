...
export default function Form() {
  ...
  const [checkboxes, setCheckboxes] = useState([
    { id: 'unifcompleto', value: 2, label: 'Si', checked: false },
    { id: 'unifparcial', value: 1, label: 'Parcialmente', checked: false },
    { id: 'sinunif', value: 0, label: 'No', checked: false },
  ]);

  // Función para actualizar el estado de las checkboxes cuando se produce un cambio

  const [checkboxes2, setCheckboxes2] = useState([
    { id: 'unifcompleto', value: 2, label: 'Sí', checked: false },
    { id: 'unifparcial', value: 1, label: 'Parcialmente', checked: false },
    { id: 'sinunif', value: 0, label: 'No', checked: false },
  ]);
  const [checkboxes3, setCheckboxes3] = useState([
    { id: 'afirmativo', value: 2, label: 'Sí', checked: false },
    { id: 'negativo', value: 0, label: 'No', checked: false },
  ]);



  const handleCheckboxesChange = (updatedCheckboxes: React.SetStateAction<{ id: string; value: number; label: string; checked: boolean; }[]>) => {
    setCheckboxes(updatedCheckboxes);
  };

  const handleCheckboxesChange2 = (updatedCheckboxes2: React.SetStateAction<{ id: string; value: number; label: string; checked: boolean; }[]>) => {
    setCheckboxes2(updatedCheckboxes2);
  };
  const handleCheckboxesChange3 = (updatedCheckboxes3: React.SetStateAction<{ id: string; value: number; label: string; checked: boolean; }[]>) => {
    setCheckboxes2(updatedCheckboxes3);
  };
    ...
    <div className=" font-general-regular text-left bg-gray-100 p-10 pb-32 ">
      ...
      <div className='ml-20 mr-60'>
        ...
        <label htmlFor="uniforme" className="block text-sm font-medium leading-6 text-gray-900">
          ¿Bitácoras de acceso a unidad?
        </label>
        <Checkboxes checkboxes={checkboxes} onCheckboxesChange={handleCheckboxesChange} />
        <label htmlFor="uniforme" className="block text-sm font-medium leading-6 text-gray-900">
          Buen registro de citados y abiertas intercalados, quienes acudieron y los que no
        </label>

        <Checkboxes checkboxes={checkboxes2} onCheckboxesChange={handleCheckboxesChange2} />
        <label htmlFor="uniforme" className="block text-sm font-medium leading-6 text-gray-900">
          Pregunta prueba
        </label>
        <Checkboxes checkboxes={checkboxes3} onCheckboxesChange={handleCheckboxesChange3} />
      </div>
      <div className='ml-20 mr-60'>
